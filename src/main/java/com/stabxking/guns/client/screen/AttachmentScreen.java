...
import com.stabxking.guns.Config;
import com.stabxking.guns.Reference;
import com.stabxking.guns.client.handler.GunRenderingHandler;
import com.stabxking.guns.client.screen.widget.MiniButton;
import com.stabxking.guns.client.util.RenderUtil;
import com.stabxking.guns.common.container.AttachmentContainer;
import com.stabxking.guns.common.container.slot.AttachmentSlot;
import com.stabxking.guns.item.GunItem;
import com.stabxking.guns.item.attachment.IAttachment;
...
{
    private static final ResourceLocation GUI_TEXTURES = new ResourceLocation("skew:textures/gui/attachments.png");
    private static final Component CONFIG_TOOLTIP = Component.translatable("skew.button.config.tooltip");
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        for(int i = 0; i < IAttachment.Type.values().length; i++)
        {
            if(RenderUtil.isMouseWithin(mouseX, mouseY, startX + 7, startY + 16 + i * 18, 18, 18))
            {
                ...
                {
                    this.renderComponentTooltip(poseStack, Arrays.asList(Component.translatable("slot.skew.attachment." + type.getTranslationKey()), Component.translatable("slot.skew.attachment.not_applicable")), mouseX, mouseY);
                }
                ...
                {
                    this.renderComponentTooltip(poseStack, Arrays.asList(Component.translatable("slot.skew.attachment.incompatible").withStyle(ChatFormatting.YELLOW)), mouseX, mouseY);
                }
                else if(this.weaponInventory.getItem(i).isEmpty())
                {
                    this.renderComponentTooltip(poseStack, Collections.singletonList(Component.translatable("slot.skew.attachment." + type.getTranslationKey())), mouseX, mouseY);
                }
            }
        }
    }
    ...
    protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY)
    {
        ...
        if(this.showHelp)
        {
            ...
            minecraft.font.draw(poseStack, I18n.get("container.skew.attachments.window_help"), 56, 38, 0xFFFFFF);
            poseStack.popPose();
        }
    }
