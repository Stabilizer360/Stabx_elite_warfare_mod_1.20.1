...
import com.stabxking.guns.GunMod;
import com.stabxking.guns.client.GunItemStackRenderer;
import com.stabxking.guns.client.KeyBinds;
import com.stabxking.guns.common.Gun;
import com.stabxking.guns.common.NetworkGunManager;
import com.stabxking.guns.debug.Debug;
import com.stabxking.guns.enchantment.EnchantmentTypes;
...
public class GunItem extends Item implements IColored, IMeta
{
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flag)
    {
        ...
        if(ammo != null)
        {
            tooltip.add(Component.translatable("info.skew.ammo_type", Component.translatable(ammo.getDescriptionId()).withStyle(ChatFormatting.WHITE)).withStyle(ChatFormatting.GRAY));
        }
        ...
        tooltip.add(Component.translatable("info.skew.damage", ChatFormatting.WHITE + ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(damage) + additionalDamageText).withStyle(ChatFormatting.GRAY));

        if(tagCompound != null)
        {
            if(tagCompound.getBoolean("IgnoreAmmo"))
            {
                tooltip.add(Component.translatable("info.skew.ignore_ammo").withStyle(ChatFormatting.AQUA));
            }
            else
            {
                int ammoCount = tagCompound.getInt("AmmoCount");
                tooltip.add(Component.translatable("info.skew.ammo", ChatFormatting.WHITE.toString() + ammoCount + "/" + GunEnchantmentHelper.getAmmoCapacity(stack, modifiedGun)).withStyle(ChatFormatting.GRAY));
            }
        }

        tooltip.add(Component.translatable("info.skew.attachment_help", KeyBinds.KEY_ATTACHMENTS.getTranslatedKeyMessage().getString().toUpperCase(Locale.ENGLISH)).withStyle(ChatFormatting.YELLOW));
    }
